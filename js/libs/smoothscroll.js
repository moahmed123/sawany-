...
;(function(document, $) {
	...
	var $scrollElement = (function() {
	    // Find out what to scroll (html or body)
	    	...
	    	if ($html.scrollTop()) {
	    		...
	    		// If scrolling the body doesnâ€™t do anything
	...
	$.fn.smoothScroll = function(speed) {
		...
		return this.find('a[href*="#"]').click(function(event) {
			...
			// If itâ€™s a link to an anchor in the same document
			if (location.pathname.replace(/^\//, '') === this.pathname.replace(/^\//, '') && location.hostname === this.hostname) {
				// If the anchor actually existsâ€¦
				if ($hash.length) {
					// â€¦donâ€™t jump to the link right awayâ€¦
					event.preventDefault();
					// â€¦and smoothly scroll to it
