...
+function ($) {
	...
	if ( $('html').attr('lang') == 'ar' ) {
		if($(window).width()<768){
			...
			$('.modal-backdrop.in').on('click',function(event){
	...
	Push.prototype.close = function () {
		$(this.options.canvas).css({'position': 'relative'}) //fix for souq / arabic / mobile
